...
public class RequestManagerHere {
    ...
    public static void sendRequest(BukkitPlayer requestor, BukkitPlayer target) {
        ...
        if (!requests.containsKey(tID)) {
            ...
            String t = Locales.TELEPORT_TPAHERE_SENT_TARGET.getString().replaceAll("%player%", Utils.replacePlayerPlaceholders(requestor.getPlayer()));
            ...
            timer.schedule(new RequestTimeOutHere(requestor, target), (reqTimeOut * 1000));
        } else {
    ...
    public static void acceptRequest(BukkitPlayer target) {
        ...
        if (requests.containsKey(tID)) {
            ...
            if (requestor.isOnline()) {
                String t = Locales.TELEPORT_TPA_ACCEPT_REQUEST.getString().replaceAll("%player%", Utils.replacePlayerPlaceholders(target.getPlayer()));
                String r = Locales.TELEPORT_TPA_ACCEPT_TARGET.getString().replaceAll("%player%", Utils.replacePlayerPlaceholders(requestor.getPlayer()));
                Messager.sendMessage(requestor.getPlayer(), r, Messager.MessageType.INFO);
                Messager.sendMessage(target.getPlayer(), t, Messager.MessageType.INFO);
                target.getPlayer().teleport(requestor.getPlayer());
            } else {
